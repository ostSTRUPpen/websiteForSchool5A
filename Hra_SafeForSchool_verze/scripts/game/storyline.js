...
function changeText(buttonNumber) {
	...
	if (textNumber == 1) {
		...
		if (retard == true) {
			document.getElementById('text_field').innerHTML =
				'Na telefonu najdeš kontakt na svého únosce (jediný kontakt v tvém telefonu) a napíšeš, že pokud tě nechá být, nebudeš na hodinách vyvolávat nikoho jiného než jeho.';
			document.getElementById('character').src =
			...
			document.getElementById('text_field').innerHTML =
				'Roztřeseně nahlásíš svůj únos policejnímu důstojníku. Dosah tvého únosce je ale větší, než sis myslel, a celá policie v tvém městě je zkorumpovaná. Policista ti nařídí rozdat ve třídě celkem 50 plusek a tím je celá záležitost uzavřena. Usínáš vyčerpáním.';
			document.getElementById('character').src =
				'../../files/pictures/asihra/characters/Adam/special/adam-scared.png';
			document.getElementById('button').style.display = 'none';
			document.getElementById('buttoon').style.display = 'none';
